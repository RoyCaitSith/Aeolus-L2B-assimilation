...
colors = [sns_cmap[0], sns_cmap[1], sns_cmap[2], sns_cmap[3], sns_cmap[4], sns_cmap[5], sns_cmap[6], sns_cmap[9]]
...
#Read GFS hgt and rh at 2021082100
GFS_time = 8
...
#Best track data from 2021081918 to 2021082206
...
pdfname = './Figure_02b_2021081900_2021082206_GFS_GOES-R_2021082100.pdf'
print(pdfname)
...
with PdfPages(pdfname) as pdf:
    ...
    time_now = datetime.datetime(2021, 8, 21,  0, 0, 0)
    ...
    file_dropsonde = '/'.join([dir_dropsondes, case, '20210820.csv'])
    ...
    ax.plot(dropsonde['CLON'].tolist()[0], dropsonde['CLAT'].tolist()[0], 'x', color='r', markersize=5.0, zorder=6)
    del dropsonde
    del df
    ...
    ax.text(np.min(lon_d01)+1.5, np.max(lat_d01)-0.75, 'D01', ha='center', va='center', color='black', fontsize=7.5, zorder=7)
    ax.text(np.min(lon_d02)+1.5, np.max(lat_d02)-0.75, 'D02', ha='center', va='center', color='black', fontsize=7.5, zorder=7)
