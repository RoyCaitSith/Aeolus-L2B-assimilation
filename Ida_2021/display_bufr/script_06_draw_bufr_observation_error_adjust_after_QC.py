    ...
    for idt, time_now in enumerate(draw_times):
        ...
        for stat in status:
            ...
            if os.path.exists(filename):
                ...
                pdfname = dir_out + '/Observation_Error_Adjust_After_QC_' + time_now_str + '_' + stat + '_' + dom + '.pdf'
                with PdfPages(pdfname) as pdf:
                    ...
                    ax = axs[0]
                    pcm = ax.scatter(latitude[mie_index_analysis], height[mie_index_analysis], c=1.0/inverse_obs_error_adjust[mie_index_analysis], \
                                     marker='|', s=25.0, vmin=0.0, vmax=5.0, cmap='gist_rainbow', zorder=0)
                    ...
                    ax = axs[1]
                    pcm = ax.scatter(latitude[rayleigh_index_analysis], height[rayleigh_index_analysis], c=1.0/inverse_obs_error_adjust[rayleigh_index_analysis], \
                                     marker='|', s=25.0, vmin=0.0, vmax=5.0, cmap='gist_rainbow', zorder=0)
                    ...
                    clb = fig.colorbar(pcm, ax=axs, ticks=np.arange(0.0, 5.1, 0.5), orientation='horizontal', pad=0.100, aspect=37.5, shrink=1.00)
                    clb.set_label('Observation Error Adjust (m/s)', fontsize=10.0, labelpad=4.0)
