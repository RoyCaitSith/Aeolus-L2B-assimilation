...
#file_wrfout_d02 = dir_wrfout + '/wrfout_d02_2021-08-26_06:00:00'
#file_wrfout_d02 = dir_wrfout + '/wrfout_d02_2021-08-26_12:00:00'
#file_wrfout_d02 = dir_wrfout + '/wrfout_d02_2021-08-26_18:00:00'
#file_wrfout_d02 = dir_wrfout + '/wrfout_d02_2021-08-27_00:00:00'
    ...
    for idt, time_now in enumerate(draw_times):
        ...
        for stat in status:
            ...
            if os.path.exists(filename):
                ...
                pdfname = dir_out + '/Observation_Before_QC_' + time_now_str + '_' + stat + '_' + dom + '.pdf'
                with PdfPages(pdfname) as pdf:

                    extent = [lat_d01[0,0], lat_d01[-1,-1], 0.0, 30]
                    #extent = [lat_d02[0,0], lat_d02[-1,-1], 0.0, 30]
                    ...
                    print(np.max([np.max( latitude[mie_index_all]), np.max( latitude[rayleigh_index_all])]))
                    print(np.min([np.min( latitude[mie_index_all]), np.min( latitude[rayleigh_index_all])]))
                    print(np.max([np.max(longitude[mie_index_all]), np.max(longitude[rayleigh_index_all])]))
                    print(np.min([np.min(longitude[mie_index_all]), np.min(longitude[rayleigh_index_all])]))
                    print(miao)
                    ...
                    mtitle = '(a) Mie Channel, Before QC'
                    ax = axs[0]
                    pcm = ax.scatter(latitude[mie_index_all], height[mie_index_all], c=observation[mie_index_all], \
                                     marker='|', s=25.0, vmin=-40.0, vmax=40.0, cmap='RdYlBu_r', zorder=0)
                    ...
                    mtitle = '(b) Rayleigh Channel, Before QC'
                    ax = axs[1]
                    pcm = ax.scatter(latitude[rayleigh_index_all], height[rayleigh_index_all], c=observation[rayleigh_index_all], \
                                     marker='|', s=25.0, vmin=-40.0, vmax=40.0, cmap='RdYlBu_r', zorder=0)
                    ...
                    clb = fig.colorbar(pcm, ax=axs, ticks=np.arange(-40.0, 41.0, 10.0), orientation='horizontal', pad=0.100, aspect=37.5, shrink=1.00)
                    clb.set_label('HLOS Wind (m/s)', fontsize=10.0, labelpad=4.0)
