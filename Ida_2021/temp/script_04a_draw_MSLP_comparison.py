...
with PdfPages(pdfname) as pdf:
    ...
    for idc, case0 in enumerate(cases0):
        ...
        mtitle = labels0[idc]
        ax.plot(np.arange(str_index, str_index+len(MSLP)-1), MSLP[:-1], 'o', color=colors0[idc], ls='--', ms=2.00, linewidth=1.25, label=mtitle, zorder=3)
    ...
    for idc, case1 in enumerate(cases1):
        ...
        mtitle = labels1[idc]
        ax.plot(np.arange(str_index, str_index+len(MSLP)-1), MSLP[:-1], 'o', color=colors1[idc], ls='-', ms=2.00, linewidth=1.25, label=mtitle, zorder=3)
        ax.plot(np.arange(str_index+idx_forecast_start_time_cycle, str_index+len(MSLP)-1, 4), MSLP[idx_forecast_start_time_cycle:-1:4], 'o', color='w', ms=0.75, zorder=3)
