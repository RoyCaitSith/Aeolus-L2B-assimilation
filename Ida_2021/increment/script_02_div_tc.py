...
cases = ['CON6h_Aeolus6h_082500_H1_Hybrid_C08', 'CON6h_Aeolus6h_082500_H2_Hybrid_C08', 'CON6h_Aeolus6h_082500_V1_Hybrid_C08', 'CON6h_Aeolus6h_082500_V2_Hybrid_C08']
labels = ['Aeolus_082500_H1_C08', 'Aeolus_082500_H2_C08', 'Aeolus_082500_V1_C08', 'Aeolus_082500_V2_C08']
...
levels = {}
levels.update({925: [-5.0, 5.1, 1.0, 'RdBu_r']})
levels.update({850: [-5.0, 5.1, 1.0, 'RdBu_r']})
levels.update({700: [-5.0, 5.1, 1.0, 'RdBu_r']})
levels.update({500: [-5.0, 5.1, 1.0, 'RdBu_r']})
levels.update({300: [-5.0, 5.1, 1.0, 'RdBu_r']})
levels.update({200: [-5.0, 5.1, 1.0, 'RdBu_r']})
levels.update({100: [-5.0, 5.1, 1.0, 'RdBu_r']})
...
for idd, dom in enumerate(domains):
    for idc, case in enumerate(cases):
        ...
        while time_now <= anl_end_time:
            ...
            tmp_ua_bkg = interplevel(ua_bkg, p_bkg, list(levels.keys()))
            tmp_va_bkg = interplevel(va_bkg, p_bkg, list(levels.keys()))
            ncfile_bkg.close()
            ...
            tmp_ua_anl = interplevel(ua_anl, p_anl, list(levels.keys()))
            tmp_va_anl = interplevel(va_anl, p_anl, list(levels.keys()))
            ncfile_anl.close()

            for idl, lev in enumerate(levels.keys()):

                figname = '_'.join([time_now_str, 'div', case, dom, str(int(lev))])
                ...
                with PdfPages(pdfname) as pdf:
                    ...
                    #print(np.nanmax(tmp_anl-tmp_bkg))
                    #print(np.nanmin(tmp_anl-tmp_bkg))
                    #print(np.nanmax(tmp_bkg))
                    #print(np.nanmin(tmp_bkg))

                    inc_contourf = ax.contourf(lon, lat, tmp_anl-tmp_bkg, \
                                               levels=np.arange(levels[lev][0], levels[lev][1], levels[lev][2]), cmap=levels[lev][3], extend='both', zorder=1)
                    #bkg_contour1 = ax.contour(lon, lat, tmp_bkg, levels=np.arange(-1025, -24.0, 50.0), linestyles='dashed', colors='k', linewidths=1.0, zorder=2)
                    #bkg_contour2 = ax.contour(lon, lat, tmp_bkg, levels=np.arange(25.0, 1001.0, 50.0), linestyles='solid',  colors='k', linewidths=1.0, zorder=2)
                    ...
                    clb.set_label('Divergence Increment ($\mathregular{10^{-5} s^{-1}}$) of Exp. ' + label + \
