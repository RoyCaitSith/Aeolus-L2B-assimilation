        ...
        while time_now <= anl_end_time:
            ...
            for idl, lev in enumerate(levels.keys()):
                ...
                with PdfPages(pdfname) as pdf:
                    ...
                    tmp_bkg[idl,:,:] = tmp_bkg[idl,:,:] - 2.0*7.2921*np.sin(lat*np.pi/180.0)
                    tmp_anl[idl,:,:] = tmp_anl[idl,:,:] - 2.0*7.2921*np.sin(lat*np.pi/180.0)
                    ...
                    #bkg_contour1 = ax.contour(lon, lat, tmp_bkg[idl,:,:], levels=np.arange(-1025, -24.0, 50.0), linestyles='dashed', colors='k', linewidths=1.0, zorder=2)
                    #bkg_contour2 = ax.contour(lon, lat, tmp_bkg[idl,:,:], levels=np.arange(25.0, 1001.0, 50.0), linestyles='solid',  colors='k', linewidths=1.0, zorder=2)
                    ...
                    clb.set_label('Relative Vorticity Increment ($\mathregular{10^{-5} s^{-1}}$) of Exp. ' + label + \
