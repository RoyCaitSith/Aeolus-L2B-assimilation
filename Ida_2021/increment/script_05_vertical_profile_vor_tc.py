...
for idd, dom in enumerate(domains):
    ...
    dir_pdf = dir_main + '/vp_vor_tc/' + cases[0]
    ...
    while time_now <= anl_end_time:
        ...
        figname = '_'.join([time_now_str, 'vertical_profile_vor', dom])
        ...
        with PdfPages(pdfname) as pdf:
            ...
            for idc, case in enumerate(cases):
                ...
                avo_bkg = getvar(ncfile_bkg, 'avo')
                lat, lon = latlon_coords(p_bkg)
                tmp_bkg = interplevel(avo_bkg, p_bkg, levels)
                ncfile_bkg.close()
                ...
                avo_anl = getvar(ncfile_anl, 'avo')
                tmp_anl = interplevel(avo_anl, p_anl, levels)
                ncfile_anl.close()
                ...
                for idlat in range(n_lat):
                    for idlon in range(n_lon):
                        ...
                        if distance <= r:
                            ...
                            print((float(lat[idlat, idlon]), float(lon[idlat, idlon]), distance))
                print(miao)

                vors = []
                for idl, lev in enumerate(levels):
                    tmp_inc = tmp_anl[idl,:,:]-tmp_bkg[idl,:,:]
                    tmp_inc_masked = np.ma.array(tmp_inc, mask=mask_latlon)
                    np.ma.masked_invalid(tmp_inc_masked)
                    vors.append(np.nanmean(tmp_inc_masked))

                ax.plot(vors, levels, '-', color=colors[idc], ms=0.25)
                ax.plot([0, 0], [0, 2000.0], '-', color='k')

            ax.set_xlabel('Increment of Vorticity ($\mathregular{10^{-5} s^{-1}}$)', fontsize=10.0)
