...
levels = {}
levels.update({925: [-0.360, 0.40, 0.08, 'BrBG']})
levels.update({850: [-0.360, 0.40, 0.08, 'BrBG']})
levels.update({700: [-0.360, 0.40, 0.08, 'BrBG']})
levels.update({500: [-0.540, 0.60, 0.12, 'BrBG']})
levels.update({300: [-0.180, 0.20, 0.04, 'BrBG']})
levels.update({200: [-.0045, .005, .001, 'BrBG']})
levels.update({100: [-.0045, .005, .001, 'BrBG']})
...
for dom in domains:
    for idc, case in enumerate(cases):
        ...
        dir_pdf = dir_main + '/qvapor_tc/' + case
        ...
        while time_now <= anl_end_time:
            ...
            var_bkg = getvar(ncfile_bkg, 'QVAPOR')
            ...
            var_anl = getvar(ncfile_anl, 'QVAPOR')
            tmp_anl = interplevel(var_anl, p_anl, list(levels.keys()))
            ncfile_anl.close()

            for idl, lev in enumerate(levels.keys()):

                figname = '_'.join([time_now_str, 'qvapor', case, dom, str(int(lev))])
                ...
                with PdfPages(pdfname) as pdf:
                    ...
                    tmp_bkg[idl,:,:] = 1000.0*tmp_bkg[idl,:,:]
                    tmp_anl[idl,:,:] = 1000.0*tmp_anl[idl,:,:]
                    ...
                    clb.set_label('QVAPOR Increment ($\mathregular{gkg^{-1}}$) of Exp. ' + label + \
