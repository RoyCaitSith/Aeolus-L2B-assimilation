    ...
    for idc, case in enumerate(cases):
        ...
        for idc, time_now in enumerate(draw_times):
            ...
            if os.path.exists(bufrname):
                ...
                pdfname = '_'.join([time_now_str, 't', case, dom+'.pdf'])
                ...
                with PdfPages(pdfname) as pdf:
                    ...
                    t_bkg = getvar(ncfile_bkg, 'temp', units='K')
                    temp_bkg = vertcross(t_bkg, ht_bkg, wrfin=ncfile_bkg, levels=level*1000.0, start_point=start_point, end_point=end_point, latlon=True, meta=True)
                    ncfile_bkg.close()
                    ...
                    t_anl = getvar(ncfile_anl, 'temp', units='K')
                    temp_anl = vertcross(t_anl, ht_anl, wrfin=ncfile_anl, levels=level*1000.0, start_point=start_point, end_point=end_point, latlon=True, meta=True)
                    ncfile_anl.close()
                    ...
                    ax = axs
                    inc_contourf = ax.contourf(lat_2d, level_2d, inc_2d, levels=np.arange(-1.0, 1.1, 0.2), cmap='RdBu_r', extend='both', zorder=1)
                    bkg_contour1 = ax.contour(lat_2d, level_2d, bkg_2d, levels=np.arange(100.0, 400.0, 4.0), linestyles='solid',  colors='k', linewidths=1.0, zorder=2)
                    ...
                    clb = fig.colorbar(inc_contourf, ax=axs, ticks=np.arange(-1.0, 1.1, 0.2), orientation='horizontal', pad=0.075, aspect=50, shrink=1.00)
                    clb.set_label('Temperature Increment (K) of Exp. ' + label + \
