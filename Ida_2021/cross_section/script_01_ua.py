    ...
    for idc, case in enumerate(cases):
        ...
        for idc, time_now in enumerate(draw_times):
            ...
            if os.path.exists(bufrname):
                ...
                level = np.arange(0, 31.0, 0.02)

                pdfname = '_'.join([time_now_str, 'ua', case, dom+'.pdf'])
                ...
                with PdfPages(pdfname) as pdf:
                    ...
                    temp_bkg = vertcross(ua_bkg, ht_bkg, wrfin=ncfile_bkg, levels=level*1000.0, start_point=start_point, end_point=end_point, latlon=True, meta=True)
                    ncfile_bkg.close()
                    ...
                    temp_anl = vertcross(ua_anl, ht_anl, wrfin=ncfile_anl, levels=level*1000.0, start_point=start_point, end_point=end_point, latlon=True, meta=True)
                    ncfile_anl.close()
                    ...
                    ax = axs
                    inc_contourf = ax.contourf(lat_2d, level_2d, inc_2d, levels=np.arange(-5.0, 5.1, 1.0), cmap='RdBu_r', extend='both', zorder=1)
                    bkg_contour1 = ax.contour(lat_2d, level_2d, bkg_2d, levels=np.arange(-66.0, -1.0, 4.0), linestyles='dashed', colors='k', linewidths=1.0, zorder=2)
                    bkg_contour2 = ax.contour(lat_2d, level_2d, bkg_2d, levels=np.arange(  2.0, 63.0, 4.0), linestyles='solid',  colors='k', linewidths=1.0, zorder=2)
                    ...
                    clb.set_label('Zonal Wind Increment (m/s) of Exp. ' + label + \
                                  ' at ' + time_now_str, fontsize=7.5, labelpad=4.0)
                    clb.ax.tick_params(axis='both', direction='in', pad=4.0, length=3.0, labelsize=10.0)
                    plt.clabel(bkg_contour1, bkg_contour1.levels[0::2], fontsize=10.0, inline=1, fmt='%1.0f')
                    plt.clabel(bkg_contour2, bkg_contour2.levels[0::2], fontsize=10.0, inline=1, fmt='%1.0f')
