    ...
    for idc, case in enumerate(cases):
        ...
        for idc, time_now in enumerate(draw_times):
            ...
            if os.path.exists(bufrname):
                ...
                pdfname = '_'.join([time_now_str, 'QVAPOR', case, dom+'.pdf'])
                ...
                with PdfPages(pdfname) as pdf:
                    ...
                    QVAPOR_bkg = getvar(ncfile_bkg, 'QVAPOR')
                    temp_bkg = vertcross(QVAPOR_bkg, ht_bkg, wrfin=ncfile_bkg, levels=level*1000.0, start_point=start_point, end_point=end_point, latlon=True, meta=True)
                    ncfile_bkg.close()
                    ...
                    QVAPOR_anl = getvar(ncfile_anl, 'QVAPOR')
                    temp_anl = vertcross(QVAPOR_anl, ht_anl, wrfin=ncfile_anl, levels=level*1000.0, start_point=start_point, end_point=end_point, latlon=True, meta=True)
                    ncfile_anl.close()
                    ...
                    factor = 1000.0
                    bkg_2d = factor*bkg_2d
                    inc_2d = factor*inc_2d

                    ax = axs
                    inc_contourf = ax.contourf(lat_2d, level_2d, inc_2d, levels=np.arange(-1.0, 1.01, 0.2), cmap='BrBG', extend='both', zorder=1)
                    bkg_contour1 = ax.contour(lat_2d, level_2d, bkg_2d, levels=np.arange(0.0, 401.0, 2.0), linestyles='solid',  colors='k', linewidths=1.0, zorder=2)
                    ...
                    clb = fig.colorbar(inc_contourf, ax=axs, ticks=np.arange(-1.0, 1.01, 0.2), orientation='horizontal', pad=0.075, aspect=50, shrink=1.00)
                    clb.set_label('Water Vapor Increment ($\mathregular{gkg^{-1}}$) of Exp. ' + label + \
