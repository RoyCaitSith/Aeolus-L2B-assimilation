...
for dom in domains:
    for case in cases:
        ...
        ncfile_output.createVariable('rvo',     'f8', ('n_time', 'n_level', 'n_lat', 'n_lon'))
        ...
        for idt in range(n_time):
            for idl in range(len(level)):
                if 'GFS' in case: ncfile_output.variables['rvo'][idt,idl,:,:] = avo[idt,idl,:,:]*100000.0 - 2.0*7.2921*np.sin(lat*np.pi/180.0)
                else: ncfile_output.variables['rvo'][idt,idl,:,:] = avo[idt,idl,:,:] - 2.0*7.2921*np.sin(lat*np.pi/180.0)
        #rel_vor = ncfile_output.variables['rvo'][:,:,:,:]
        ncfile_output.close()
