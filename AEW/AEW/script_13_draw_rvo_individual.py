...
for dom in domains:
    for idt in range(n_time):
        ...
        for idc, case in enumerate(cases):
            ...
            filename = dir_file + '/' + case + '/rvo_' + dom + '.nc'
            ncfile   = Dataset(filename)
            lat      = ncfile.variables['lat'][:,:]
            lon      = ncfile.variables['lon'][:,:]
            level    = ncfile.variables['level'][:]
            rvo      = ncfile.variables['rvo'][time_idx,:,:,:]
            ncfile.close()

            pdfname = '_'.join([time_now_str, 'rvo', case, dom+'.pdf'])
            ...
            with PdfPages(pdfname) as pdf:
                for idl, lev in enumerate(levels):
                    ...
                    pcm = ax.contourf(lon, lat, rvo[idl,:,:], levels=np.arange(-9.0, 10.0, 2.0), cmap='RdBu_r', extend='both', zorder=1)
