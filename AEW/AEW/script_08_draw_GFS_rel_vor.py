...
for dom in domains:
    ...
    rel_vor  = ncfile.variables['rel_vor'][:,:,:,:]*10000.0
    ncfile.close()

    extent = [lon[0,0], lon[-1,-1], lat[0,0], lat[-1,-1]]

    pdfname = dir_file + '/rel_vor_' + dom + '.pdf'
    with PdfPages(pdfname) as pdf:
        for idt in range(0, n_time):
            ...
            suptitle_str = time_now_str + ', ' + dom + ', relative vorticity'
            ...
            for idl, lev in enumerate(level[0:3]):
                ...
                pcm = ax.contourf(lon, lat, rel_vor[idt,idl,:,:], levels=np.arange(-0.9, 1.0, 0.2), cmap='RdBu_r', extend='both', zorder=0)
