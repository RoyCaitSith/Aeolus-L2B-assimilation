...
for dom in domains:
    for idt in range(n_time):
        ...
        for idc, case in enumerate(cases):
            ...
            filename = dir_file + '/' + case + '/rh_' + dom + '.nc'
            ncfile   = Dataset(filename)
            lat      = ncfile.variables['lat'][:,:]
            lon      = ncfile.variables['lon'][:,:]
            level    = ncfile.variables['level'][:]
            rh       = ncfile.variables['rh'][time_idx,:,:,:]
            ncfile.close()
            ...
            with PdfPages(pdfname) as pdf:
                for idl, lev in enumerate(levels):
                    ...
                    pcm = ax.contourf(lon, lat, rh[idl,:,:], levels=np.arange(0.0, 100.1, 10.0), cmap='YlGn', extend='both', zorder=1)
                    ...
                    clb.set_label('Relative Humidity (%) of ' + label + ' at ' + str(lev) + ' hPa', fontsize=10.0, labelpad=4.0)
                    clb.ax.tick_params(axis='both', direction='in', pad=4.0, length=3.0, labelsize=10.0)
                    clb.set_ticks(np.arange(0.0, 100.1, 10.0))
