...
for dom in domains:
    ...
    rel_vor_gau = ncfile.variables['rel_vor_gau'][:,:,:,:]*10000.0
    ncfile.close()
    ...
    pdfname = dir_file + '/rel_vor_gau_' + dom + '.pdf'
    with PdfPages(pdfname) as pdf:
        for idt in range(0, n_time):
            ...
            suptitle_str = time_now_str + ', ' + dom + ', relative vorticity after gaussian filter'
            ...
            for idl, lev in enumerate(level[0:3]):
                ...
                #print(argrelmax(rel_vor_gau[idt,idl,:,:], order=100))
                pcm = ax.contourf(lon, lat, rel_vor_gau[idt,idl,:,:], levels=np.arange(-0.25, 0.26, 0.05), cmap='RdBu_r', extend='both', zorder=0)
            ...
            clb.set_label('relative vorticity after gaussian filter $\mathregular{10^{-4} s^{-1}}$', fontsize=7.5, labelpad=4.0)
