...
for dom in domains:
    ...
    #avo      = np.mean(ncfile.variables['avo'][:,:, 95:187,:], axis=2)*10000.0
    avo      = np.mean(ncfile.variables['avo'][:,:,131:225,:], axis=2)*10000.0
    ncfile.close()
    ...
    pdfname = dir_file + '/Hovmoller_avo_' + dom + '.pdf'
    with PdfPages(pdfname) as pdf:
        ...
        suptitle_str = start_time_str + ', ' + end_time_str + ', ' + dom + ', abs_vor'
        fig.suptitle(suptitle_str, fontsize=7.5)

        for idl, lev in enumerate(level[0:3]):

            ax = axs[idl]
            pcm = ax.contourf(Xi, Yi, avo[::-1,idl,:], levels=np.arange(-0.90, 1.00, 0.2), cmap='RdBu_r', extend='both', zorder=0)
        ...
        clb.set_ticks(np.arange(-0.90, 1.00, 0.2))
