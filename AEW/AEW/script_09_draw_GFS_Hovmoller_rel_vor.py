...
for dom in domains:
    ...
    #rel_vor  = np.mean(ncfile.variables['rel_vor'][:,:, 95:187,:], axis=2)*10000.0
    rel_vor  = np.mean(ncfile.variables['rel_vor'][:,:,131:225,:], axis=2)*10000.0
    ncfile.close()

    xi = lon[0, :]
    yi = np.arange(0, n_time)
    Xi, Yi = np.meshgrid(xi, yi)
    extent = [lon[0,0], lon[-1,-1], 0, 12]

    pdfname = dir_file + '/Hovmoller_rel_vor_' + dom + '.pdf'
    with PdfPages(pdfname) as pdf:
        ...
        suptitle_str = start_time_str + ', ' + end_time_str + ', ' + dom + ', rel_vor'
        fig.suptitle(suptitle_str, fontsize=7.5)

        for idl, lev in enumerate(level[0:3]):

            ax = axs[idl]
            pcm = ax.contourf(Xi, Yi, rel_vor[::-1,idl,:], levels=np.arange(-0.25, 0.26, 0.05), cmap='RdBu_r', extend='both', zorder=0)
        ...
        clb.set_label('relative vorticity $\mathregular{10^{-4} s^{-1}}$', fontsize=7.5, labelpad=4.0)
        clb.ax.tick_params(axis='both', direction='in', pad=4.0, length=3.0, labelsize=7.5)
        clb.set_ticks(np.arange(-0.25, 0.26, 0.05))
