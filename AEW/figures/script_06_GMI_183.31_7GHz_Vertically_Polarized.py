...
for case in cases:
    ...
    for (idt, time_now) in enumerate(draw_times):
        ...
        pdfname = '_'.join([case, time_now_str, 'GMI_183.31_7GHz_Vertically_Polarized.pdf'])
        print(pdfname)
        ...
        with PdfPages(pdfname) as pdf:
            ...
            pcm = ax.contourf(x_s2_lon, y_s2_lat, S2_earthView[:,:,3]/100.0, levels=np.arange(230, 290.1, 1.0), cmap=cpt_convert_r, extend='both', zorder=1)
            if if_AEW:
            ...
            clb_label = 'GMI 183.31_7 GHz V Brightness Temperatures (K)'
            clb = fig.colorbar(pcm, ax=ax, ticks=np.arange(230, 290.1, 10), orientation='horizontal', pad=0.035, aspect=50, shrink=1.00)
