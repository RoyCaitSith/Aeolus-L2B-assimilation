...
!    Extract type, date, and location information
     call ufbint(lunin,  header, 4, 1, iret, trim( headerstr))
     call ufbint(lunin,  yymmdd, 3, 1, iret, trim( yymmddstr))
     call ufbint(lunin,  hhmmss, 3, 1, iret, trim( hhmmssstr))
     call ufbrep(lunin,  l2bgeo, 8, 6, iret, trim( l2bgeostr))
     call ufbint(lunin, l2binfo, 8, 1, iret, trim(l2binfostr))
     call ufbint(lunin,  l2bptb, 6, 1, iret, trim( l2bptbstr))
...
!    Determine if this is aeolus L2B report
     dwl= (ikx /= 0) .and. (subset=='ALADIN2B')
     if(.not. dwl) then
          cycle obsloop
     endif
     ...
     t4dv=(real((minobs-iwinbgn),r_kind) + (hhmmss(3)+l2bgeo(4,3))*r60inv)*r60inv
     if (l4dvar.or.l4densvar) then
        if (t4dv<zero .OR. t4dv>winlen) cycle obsloop
     else
        time=(real((minobs-minan),r_kind) + (hhmmss(3)+l2bgeo(4,3))*r60inv)*r60inv
        if (abs(time) > ctwind(ikx) .or. abs(time) > twind) cycle obsloop
     endif

     rstation_id=header(1)

     l2bgeo(3,3)=mod(l2bgeo(3,3),r360)
     if (l2bgeo(3,3) < zero)  l2bgeo(3,3)=l2bgeo(3,3)+r360

     dlat_earth_deg = l2bgeo(2,3)
     dlon_earth_deg = l2bgeo(3,3)
     dlat_earth = l2bgeo(2,3) * deg2rad
     dlon_earth = l2bgeo(3,3) * deg2rad
...
!    If wind data, extract observation.
     ...
     cdata_all(5,ndata)=l2bgeo(5,6)            ! obs height (altitude) (m)
     cdata_all(6,ndata)=l2bgeo(7,6)*deg2rad    ! elevation angle (radians)
     cdata_all(7,ndata)=l2bgeo(6,6)*deg2rad    ! bearing or azimuth (radians)
     cdata_all(8,ndata)=l2binfo(3)             ! receiver channel
     cdata_all(9,ndata)=l2binfo(4)             ! lidar L2b classification type
     cdata_all(10,ndata)=l2binfo(8)            ! confidence flag
     cdata_all(11,ndata)=l2binfo(6)            ! obs wind (line of sight component) msq
     cdata_all(12,ndata)=l2binfo(7)            ! standard deviation (obs error) msq
