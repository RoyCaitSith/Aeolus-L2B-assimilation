...
!    index information for data array (see reading routine)
  ...
  ihgt=5      ! index of obs height (altitude) (m)
  ielva=6     ! index of elevation angle(radians)
  iazm=7      ! index of azimuth angle(radians) in data array
  inls=8      ! index of number of receiver channel
  incls=9     ! index of number of lidar L2b classification type
  iatd=10     ! index of confidence flag
...
! Set observation error to 2.5 m/s for Mie Cloudy Data
     if(data(inls, i) .eq. 0) then
        error = 2.5
     endif

! Set observation error to 4.5 m/s for Rayleigh Clear Data
     if(data(inls, i) .eq. 1) then
        error = 4.5
     endif

! Inflation of the observation error
!     error = error + 2.00
...
!    Quality Control
!    Only use Mie Cloudy and Rayleigh Clear data
     if (((data(inls, i) .eq. 0) .and. (data(incls, i) .ne. 1)) .or. ((data(inls, i) .eq. 1) .and. (data(incls, i) .ne. 0))) then
        error = zero
        ratio_errors = zero
     endif

!    Remove invalid data
     if (data(iatd, i) .ne. 0) then
        error = zero
        ratio_errors = zero
     endif

!    Remove data whose abs(HLOS) > 1000 m/s
     if (abs(data(ilob, i)) > 1000.0) then
        error = zero
        ratio_errors = zero
     endif

!    Do not use any Mie winds with estimated errors (before scaling) > 10 m/s
     if ((data(inls, i) .eq. 0) .and. ((data(ier, i) .gt. 10))) then
        error = zero
        ratio_errors = zero
     endif
     
!    Do not use any Rayleigh winds with estimated errors (before scaling) > 12 m/s
     if ((data(inls, i) .eq. 1) .and. ((data(ier, i) .gt. 12))) then
        error = zero
        ratio_errors = zero
     endif
...
900  format('conv_dw_',i2.2,'.nc4')
  ...
  subroutine contents_binary_diag_
        ...
        rdiagbuf(23,ii) = data(inls,i)         ! number of receiver channel
        rdiagbuf(24,ii) = data(incls,i)        ! number of lidar L2b classification type
        rdiagbuf(25,ii) = data(iatd,i)         ! confidence flag
...
!_RT    rdiagbuf(23,ii) = data(inls,i)         ! number of receiver channel
!_RT    rdiagbuf(24,ii) = data(incls,i)        ! number of lidar L2b classification type
!_RT    rdiagbuf(25,ii) = data(iatd,i)         ! confidence flag
