...
!  Note other module variables are only used within this routine
  ...
  real(r_kind)   , save ,allocatable,dimension(:,:) :: cloud_mr     ! cloud mixing ratio fed into CRTM 
...
!$$$
  ...
  use crtm_module, only: mass_mixing_ratio_units,co2_id,o3_id,crtm_init, &
      ...
      volume_mixing_ratio_units,h2o_id,ch4_id,n2o_id,co_id,tropical
...
! When Cloud is available in MetGuess, defined Cloudy Radiance
 ...
 if (radmod%lcloud_fwd) then
    ...
    allocate(cloud_mr(msig,n_clouds_fwd))
...
!$$$
  ...
  if(allocated(cloud_mr)) deallocate(cloud_mr)
...
!                     cloud_cont(k,ii)=max(1.001_r_kind*1.0E-6_r_kind, cloud_cont(k,ii))
                     cloud_cont(k,ii)=cloud_cont(k,ii)
...
! Set clouds for CRTM
  if(n_clouds_fwd_wk>0) then
     ...
     call Set_CRTM_Cloud (msig,n_actual_clouds_wk,cloud_names,icmask,n_clouds_fwd_wk,cloud_cont,cloud_mr,cloud_efr,jcloud,auxdp, &
