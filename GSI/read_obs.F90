...
!        May not be necessary with the operational satwnd BUFR
       ...
       else if(trim(filename) == 'halobufr'    .or. &
               trim(filename) == 'dsbufr'      .or. &
               trim(filename) == 'tropicsbufr' .or. &
...
!               Process flight-letel high-density data not included in prepbufr
                if ( index(infile,'halobufr') /=0 .or. index(infile,'dsbufr') /=0 ) then
                  ...
                  call read_tropics(nread,npuse,nouse,infile,obstype,lunout,gstime,twind,sis,&
                                    prsl_full,nobs_sub1(1,i))
                  string='READ_TROPICS'
                else
...
!             Process rapidscat winds which seperate from prepbufr
                ...
                else if ( index(infile,'hdobbufr') /=0 .or. index(infile,'dsbufr') /=0 ) then
                  call read_fl_hdob(nread,npuse,nouse,infile,obstype,lunout,gstime,twind,sis,&
                       prsl_full,nobs_sub1(1,i))
                  string='READ_FL_HDOB'
                else
