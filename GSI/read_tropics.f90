subroutine read_tropics(nread,ndata,nodata,infile,obstype,lunout,gstime,twind,sis,&
                        prsl_full,nobs)

!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:  read_tropics            read obs from hdob bufr file
...
!------------------------------------------------------------------------------------------------
     ...
     write(6,*)'READ_TROPICS: begin to read flight-level high density data ...'
...
!    Set common variables
     ...
     if (ltob) then
        ...
        write(6,*) ' illegal obs type in tropics '
        call stop2(94)
     end if
     if (perturb_obs .and. (ltob .or. lqob)) nreal = nreal+1
...
!     if (lcount <= 0) then
!        write(6,*)'READ_TROPICS: obs error table not available'
!        call stop2(49) 
!     else
!        write(6,*)'READ_TROPICS: obs errors provided by local file errtable'   
!     endif
!
...
!    If found, get the index (nc) from the convinfo for the specified type
     ...
     if(ntmatch == 0)then  ! Return if not specified in convinfo 
        write(6,*) ' READ_TROPICS: No matching obstype found in obsinfo ',obstype
        return
     else 
        nc = ncsave
        write(6,*) ' READ_TROPICS: Processing FL HDOB data : ', ntmatch, nc, ioctype(nc), ictype(nc), itype 
     end if
...
!    Setup thinning parameters
     ...
     if (ithin > 0) then
        ...
        write(6,*)'READ_TROPICS: ictype(nc),rmesh,pflag,nlevp,pmesh,nc ',&
...
!    Go through the bufr file to find out how mant subsets to process
     ...
     call closbf(lunin)
     write(6,*) 'READ_TROPICS: total number of data found in the bufr file ',maxobs,obstype      
     write(6,*) 'READ_TROPICS: time offset is ',toff,' hours'
...
!                 3  both lat/lon abd GA/PS questionable
           ...
           if (obsqcm(1,1) > 0) then
              write(6,*) 'READ_TROPICS: bad positional data ... toss away'
              cycle loop_readsb2
           endif

!          Read QC mark for HDOB meteorological status
!          QHDOM: 0  all parameters of nominal accuracy
...
!          Read observation location (lat/lon degree) 
           ...
           if (obsloc(1,1) == missing .or. abs(obsloc(1,1)) >  90.0_r_kind .or. &     
               ...
               write(6,*) 'READ_TROPICS: bad lat/lon values: ', obsloc(1,1),obsloc(2,1)              
               cycle loop_readsb2     
           endif
...
!          Read flight-level pressure [Pa] and convert to [cb]
           ...
           if (obsprs(1,1) >= missing) then            
               write(6,*) 'READ_TROPICS: bad flight-level pressure [pa] values: ', obsprs(1,1)          
               cycle loop_readsb2     
           endif
...
!          Read flight-level geopotential height [(m/s)**2] and convert to height [m]       
           ...
           if (obsg10(1,1) >= missing) then                                  
               write(6,*) 'READ_TROPICS: bad flight-level geopotential height [(m/s)**2] values: ', obsg10(1,1)
               cycle loop_readsb2 
           endif    
              ...
              if (njqc) then
                 ...
                 do i =1,maxsub_t
                    if( icsubtype(nc)  == isuble_t(itypey,i) ) then
                       ...
                       enddo
                       if(ncount_t ==1) then
                          write(6,*) 'READ_TROPICS,WARNING!! tob:cannot find subtyep in the error table,&
              ...
              if (njqc) then
                 ...
                 do i =1,maxsub_q
                    if( icsubtype(nc)  == isuble_q(itypey,i) ) then
                       ...
                       enddo
                       if( ncount_q ==1 ) then
                          write(6,*) 'READ_TROPICS,WARNING!! qob:cannot find subtyep in the error table,&
                                      itype,iosub=',itype,icsubtype(nc) 
...
900  continue
     if(diagnostic_reg .and. ntest>0)  write(6,*)'READ_TROPICS:  ',&
        'ntest,  disterrmax=', ntest,disterrmax
     if(diagnostic_reg .and. nvtest>0) write(6,*)'READ_TROPICS:  ',&
        'nvtest,vdisterrmax=',ntest,vdisterrmax

     if (ndata == 0) then
        call closbf(lunin)
        write(6,*)'READ_TROPICS: no data to process'
     endif
     write(6,*)'READ_TROPICS: nreal=',nreal
     write(6,*)'READ_TROPICS: ntb,nread,ndata,nodata=',ntb,nread,ndata,nodata
